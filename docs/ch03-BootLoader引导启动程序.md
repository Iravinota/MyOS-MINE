...
12. *开始执行boot.asm程序*
13. 参看[第10节 boot.asm启动过程总结](#10-bootasm启动过程总结)
14. *boot.asm程序执行结束*，CPU跳转至0x1000:0000物理地址处继续执行loader.bin
15. loader.bin执行过程参看[第11节 loader.asm从实模式进入保护模式以及IA-32e模式并加载kernel.bin过程](#11-loaderasm从实模式进入保护模式以及ia-32e模式并加载kernelbin过程)
