...
- *main.c* 中执行`task_init()`，来初始化 **0号进程** 和 **1号进程init**
  ...
  - **1号进程** init的初始化过程： 新申请一个内存页，复制当前进程的struct task_struct结构，在之上新建struct thread_struct结构；构造一个执行现场放到新进程的栈中，其中rbx执行新进程的执行代码入口，rdx保存传入参数；设置rip，若为内核进程，则指向kernel_thread_func，若为应用层进程，则指向ret_from_intr；当进程切换完成后，执行rip指向的位置的代码；开始执行新进程
